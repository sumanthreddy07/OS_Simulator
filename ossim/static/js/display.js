...
function drawTable(i){
    ...
    console.log(a[i]);
    $("#ptable").append("<tr><td>"+a[i].no+"</td><td>"+a[i].at+"</td><td>"+a[i].bt+"</td><td>"+a[i].ct+"</td><td>"+a[i].wt+"</td><td>"+a[i].tat+"</td></tr>");
  }
...
function displayBlock(i){
  ...
  if(sequence[i].no != -1)
  $('#process-'+sequence[i].start).css('background-color',colors[i]);
